...
def _ensure_state(game: Game) -> GameState:
    ...
    if not st.data:
        st.data = {"turn": 1, "phase": "SETUP", "board": {}}
        st.save()
    return st
...
class SetupAPI(APIView):
    ...
    def post(self, request, game_id):
        ...
        eng = Engine(st.data); me=_actor(g, request.user)
...
class GameByCode(APIView):
    ...
    def get(self, request, code):
        ...
        st = _ensure_state(g)
        return Response({"id": str(g.id), "state": st.data})
...
class MyGames(APIView):
    ...
    def get(self, request):
        ...
        for g in qs:
            ...
            items.append({"id": str(g.id), "code": g.code, "opponent": opp_login, "status": g.status})
